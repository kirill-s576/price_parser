...
(function($) {
    ...
    $.fn.tsRotate = function(options) {
        ...
        var zoomDiv = settings.zoom ? '<div class="zoom"><i class="jl360-zoom-in"></i></div>' : "";
        ...
        holder.html("<img class='ts_img_view' src='' /><img class='ts_imgzoom_view' src='' />" + pointerDiv + zoomDiv + "<div class='loading_bg'><div class='loading'><p>loading</p><div class='loading_bar'><div class='loading_bar_inside'></div></div></div></div><div class='zoomload_bg'><div class='zoomload_gif'></div></div></div>" +
            addLogo);
        ...
        var zoomLoadLeft = Math.round((contWidth -
            220) / 2);
        ...
        var countFrames;
        var zoomon;
        if (!nophp) $.getJSON(pathtophp + "/loadimages.php", {
            fname: imgpath,
            zoomdir: zoomfolder
        }, function(output) {
        ...
        function preload(arrayOfImages, countFrames) {
            ...
            if (hotspots) {
                ...
                if (counthotspots < 1) alert("Please add at least one hotspot element (see manual)");
                else {
                    toti = new Array;
        ...
        if (isautorotate) {
            ...
            if (is_touch_device) holder.on("touchstart", function() {
                stopautorotate()
            })
        }
        if (hotspots) {
            ...
            holder.on("click", ".highlights_item:not(.active)", function(e) {
                ...
                } else if (expanded) collapseHighlight(function() {
                    expandHighlight(itemid)
                });
                else expandHighlight(itemid)
            });
            holder.on("click", ".hotspot:not(.expanded)", function(e) {
                e.stopPropagation();
                var itemid =
                    parseInt($(this).attr("id").replace("hs", ""));
                if (expanded) collapseHighlight(function() {
                    expandHighlight(itemid)
                });
                else expandHighlight(itemid)
            });
            if (!is_touch_device) holder.on("click", ".expanded", function(e) {
                e.stopPropagation();
                holder.find(".highlights_item").removeClass("active");
                collapseHighlight()
            });
            if (is_touch_device) {
                ...
                holder.on("touchstart", ".highlights",
                    function(e) {
                        e.stopPropagation();
                        e.preventDefault();
                        $(this).find(".highlights_item").css({
            ...
            function showHighlights(itemid) {
                ...
                for (var i = 0; i < toti.length; i++)
                    if (toti[i].frame == currentFrame) {
                        ...
                        if (toti[i].posix > 60) holder.find("#hs" + i + "").addClass("posr");
                        if (toti[i].posiy > 75) holder.find("#hs" + i + "").addClass("posb")
                    } if (itemid !== undefined) expandHighlight(itemid);
            ...
            function hideHighlights() {
                if (expanded) collapseHighlight(function() {
                    holder.find(".highlights_item").removeClass("active");
                    if (hsFade) holder.find(".hotspot").fadeOut(150, function() {
                        $(this).remove()
                    });
                    else {
                        holder.find(".hotspot").css({
                            "display": "none"
                        });
                        $(this).remove()
                    }
                ...
                if (itemid !== undefined) {
                    ...
                    if (toti[itemid].text)
                        if (holder.find("#hs" + itemid + "").is(".posb"))
                            if (toti[itemid].link) holder.find("#hs" + itemid + "").prepend('<a class="hltext link" href="' + toti[itemid].link + '" target="' + toti[itemid].target + '">' + toti[itemid].text + "</a>");
                            else holder.find("#hs" + itemid + "").prepend('<span class="hltext">' + toti[itemid].text +
                                "</span>");
                    else if (toti[itemid].link) holder.find("#hs" + itemid + "").append('<a class="hltext link" href="' + toti[itemid].link + '" target="' + toti[itemid].target + '">' + toti[itemid].text + "</a>");
                    ...
                    if (holder.find("#hs" + itemid + "").is(".posr")) {
                        ...
                        holder.find("#hs" + itemid + "").css({
                            "left": "0px"
                        });
                        var hswidth = holder.find("#hs" + itemid + "").outerWidth();
                        var newhsposition =
                            hsposition.left - hswidth + 20;
                        holder.find("#hs" + itemid + "").css({
                            "left": newhsposition + "px"
                        })
                    }
                    if (holder.find("#hs" + itemid + "").is(".posb")) {
                        var hsposition = holder.find("#hs" + itemid + "").position();
            ...
            function collapseHighlight(callback) {
                var itemid = parseInt(holder.find(".expanded").attr("id").replace("hs",
                    ""));
                holder.find(".expanded").css({
                    "top": toti[itemid].posiy + "%",
                    "left": toti[itemid].posix + "%"
                }).removeClass("expanded").find(".hltitle,.hltext").remove();
                expanded = false;
        ...
        function startautorotate(rtl) {
            playing = true;
            if (!rtl) autorotate = setInterval(nextFrame, autorotatespeed);
            if (rtl) autorotate = setInterval(prevFrame, autorotatespeed);
            if (playstop) $("." + thisName + "." + playstop + "").addClass("busy")
        }
        ...
        function rotateImg(enterPosition) {
            doc.on("mousemove.dragrotate", function(e) {
                if (changeAxis) var cursorPosition = e.pageY - contOffset.top;
                else var cursorPosition =
                    e.pageX - contOffset.left;
                ...
                if (hotspots)
                    if (currentFrame != startFrame) hideHighlights();
                if (currentFrame >= countFrames) currentFrame = currentFrame - countFrames * cycles;
                if (currentFrame < 0) currentFrame = countFrames * cycles + currentFrame;
                image.attr("src", imagelist[currentFrame]);
                if (!changeAxis) setPointer()
            });
            doc.on("mouseup.dragrotate", function() {
                if (hotspots && highlightsHidden) showHighlights();
                startFrame = currentFrame;
                doc.off(".dragrotate")
            });
            if (rotatehover) holder.on("mouseleave.dragrotate", function() {
                if (hotspots && highlightsHidden) showHighlights();
                startFrame = currentFrame;
                doc.off(".dragrotate")
            })
        ...
        function rotateImgMobile(enterPosition) {
            holder.on("touchmove.dragrotatemob", function(mobileEvent) {
                ...
                if (changeAxis) var cursorPosition = event.touches[0].pageY - contOffset.top;
                else var cursorPosition = event.touches[0].pageX - contOffset.left;
                var xOffset = cursorPosition - enterPosition;
                var step = Math.round(contWidth / countFrames) * direction;
                var frameOffset = Math.round(xOffset / step);
                var cycles = Math.abs(Math.floor((frameOffset + startFrame) / countFrames));
        ...
        function zoomImg(startXpos, startYpos, offset) {
            ...
            if (zoomImg.complete || zoomImg.readystate ===
                4);
            else holder.find(".zoomload_bg").fadeIn();
            zoomImg.onload = function() {
                ...
                holder.on("mousemove.dragpan", function(e) {
                    ...
                    if (!hMoveLock) imagezoom.css("left", leftOffset);
                    if (!vMoveLock) imagezoom.css("top", topOffset)
                });
                doc.on("mousedown.zoomof", function() {
                    if (!zoomloading) zoomOut()
                })
            }
        }
        ...
        function zoomMoveMobile(startXpos, startYpos, offset, leftOverflow, topOverflow) {
            ...
            if (leftOffset <= xlimit) leftOffset = xlimit;
            if (topOffset >= 0) topOffset = 0;
            if (topOffset <= ylimit) topOffset = ylimit;
        ...
        function zoomImgMobile(offset) {
            ...
            if (hotspots) hideHighlights();
            holder.find(".highlights").fadeOut();
            $(document).find(".highlights_item").css({
                "display": "none"
            });
            var zoomloading = true;
            holder.find(".round").fadeOut();
            ...
            zoomImg.onload = function() {
                ...
                zoomWidth = zoomImg.width;
                var leftOverflow = (zoomWidth - contWidth) / -2;
                var topOverflow = (zoomHeight - contHeight) /
                    -2;
                imagezoom.attr("src", zoomlist[currentFrame]);
                imagezoom.css({
                    "left": leftOverflow,
                ...
                holder.on("touchstart.dragstartmob", function() {
                    if (typeof newleftOverflow !==
                        "undefined") leftOverflow = newleftOverflow;
                    if (typeof newtopOverflow !== "undefined") topOverflow = newtopOverflow;
        ...
        holder.on("mousedown.initrotate",
            function(e) {
                if (!zoomon) {
                    if (changeAxis) var enterPosition = e.pageY - contOffset.top;
                    else var enterPosition = e.pageX - contOffset.left;
                    rotateImg(enterPosition)
                }
            });
        if (wheelRotate) holder.on("mousewheel", function(e) {
            if (!zoomon) {
                e.preventDefault();
                if (playing) stopautorotate();
                if (e.originalEvent.wheelDelta / 120 > 0) nextFrame();
                else prevFrame()
            }
        });
        if (rotatehover) holder.on("mouseenter.initrotate", function(e) {
            if (!zoomon) {
                if (changeAxis) var enterPosition = e.pageY - contOffset.top;
                else var enterPosition = e.pageX - contOffset.left;
                rotateImg(enterPosition)
        ...
        if (is_touch_device) {
            holder.find(image).on("touchstart.initrotatemob", function(mobileEvent) {
                if (hotspots) $(document).find(".highlights_item").css({
                    "display": "none"
                });
                if (!zoomon) {
                    mobileEvent.preventDefault();
                    var sevent = window.event;
                    if (changeAxis) var enterPosition = sevent.touches[0].pageY - contOffset.top;
                    else var enterPosition = sevent.touches[0].pageX - contOffset.left;
        ...
        function nextFrame() {
            if (hotspots) hideHighlights();
            currentFrame++;
            if (currentFrame >= countFrames) currentFrame = 0;
            image.attr("src", imagelist[currentFrame]);
            if (!changeAxis) setPointer();
            startFrame = currentFrame;
            if (hotspots) showHighlights();
            if (!isrotateloop) {
                autorotateFrame =
                    autorotateFrame + 1;
                if (autorotateFrame == countFrames) stopautorotate()
            }
        }
        ...
        function prevFrame() {
            if (hotspots) hideHighlights();
            currentFrame--;
            if (currentFrame < 0) currentFrame = countFrames;
            image.attr("src", imagelist[currentFrame]);
            if (!changeAxis) setPointer();
            startFrame = currentFrame;
            if (hotspots) showHighlights()
        ...
        if (playstop) $("." + thisName + "." + playstop + "").on("click", function() {
            if (playing) stopautorotate();
...
/* JLex 360 extend by above scripts */
function jlex360(s) {
	var $j = jQuery.noConflict();
	$j(document).ready(function(){
		s.playstopclass = 'playstop';
		s.rightclass = 'gright';
		s.leftclass = 'gleft';
		var $parent = $j('#'+s.id).parent(),
			to = null;

		var cb=function(){
			if ($parent.width()*1 < s.width) {
				s.height = parseInt( $parent.width()*s.height/s.width );
				s.width = $parent.width();
			}

			$j('#'+s.id).attr('width',s.width).attr('height',s.height);
			$j('#'+s.id).tsRotate(s);
		};
		...
		$j(window).resize(function(){
			//j('#holder_'+s.id).removeAttr('style');
		});
	});
}
