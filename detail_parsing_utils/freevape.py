...
class Freevape(object):
    def __init__(self):
        # Переменные магазина
        self.www = "https://freevape.by/"
        ...
        self.name_regexp = r'<div class="woocommerce-product-details__short-description"><h3>(.*?)</h3>'
        self.price_regexp = r'</h1><p class="price"><span class="woocommerce-Price-amount amount">(.*?)&nbsp;<span'
        self.price_regexp1 = r'</h1><p class="price">.*?<ins><span class="woocommerce-Price-amount amount">(.*?)&nbsp'
        self.available_regexp = r'<p class="stock (.*?)">.*?</p>'
        self.fullprice_regexp = r'</h1><p class="price"><del><span class="woocommerce-Price-amount amount">(.*?)&nbsp'
        self.imageurl_regexp = r'class="woocommerce-product-gallery__image"><a href="(.*?)">'
    ...
    def parse_price(self):
        try:
            ...
            self.goose_price = float(
                str(re.findall(self.price_regexp1, self.goose_html, flags=re.DOTALL)[0]).replace(",", "."))
        return self

    def parse_available(self):
        self.goose_available = re.findall(self.available_regexp, self.goose_html, flags=re.DOTALL)
        if "out" not in self.goose_available:
    ...
    def parse_fullprice(self):
        try:
            self.goose_fullprice = float(str(re.findall(self.fullprice_regexp, self.goose_html, flags=re.DOTALL)[0]).replace(",", "."))
        except:
